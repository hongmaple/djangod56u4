...
def caipinxinxi_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if req_dict.get("mima") and "mima" not in caipinxinxi.getallcolumn(caipinxinxi,caipinxinxi) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in caipinxinxi.getallcolumn(caipinxinxi,caipinxinxi) :
...
def caipinxinxi_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'caipinxinxi' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from caipinxinxi where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from caipinxinxi where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def caipinxinxi_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM caipinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM caipinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM caipinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, yColumnName, where, '%Y')
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def caipinxinxi_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM caipinxinxi {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def caipinxinxi_group(request, columnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        
        where = ' where 1 = 1 '

        sql = "SELECT COUNT(*) AS total, " + columnName + " FROM caipinxinxi " + where + " GROUP BY " + columnName + " LIMIT 10" 
